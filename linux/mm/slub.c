...
bool find_cache_from_address(const void *addr, const char *desc) {
	if ((unsigned long long)addr < 0x0000800000000000) {
		printk("[0x%lx]->user-space(%s)\n", 
		(unsigned long)addr, desc);
		return true;
	}

	struct folio *folio = virt_to_folio(addr);
	struct slab *slab = (void *)folio;
	if ((unsigned long long)slab < 0xffff888000000000 ||
		(unsigned long long)slab->slab_cache < 0xffff888000000000 ||
		(unsigned long long)slab->slab_cache->name < 0xffff888000000000) {
		// printk("[0x%lx]->no-slab(%s)\n", 
		// (unsigned long)addr, desc);
		return false;
	}
	printk("[0x%lx]->%s(%s)\n", 
		(unsigned long)addr,
		slab->slab_cache->name,
		desc);
    return true;
}