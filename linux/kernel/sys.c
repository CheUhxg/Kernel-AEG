...
static pid_t focus_pids[FUZZ_PID_NUM];

static void insert_infos(const struct log_cap_info *info) {
	int i;

	if (infos.size == INFO_MAX_NUM)
		return;

	for (i = 0; i < infos.size; ++i) {
		if (memcmp(&infos.buf[i], info, sizeof(struct log_cap_info)) == 0)
			return;
	}
	memcpy(&infos.buf[infos.size++], info, sizeof(struct log_cap_info));
}

bool is_local_focus_equal(void)
{
	int i;

	for (i = 0; i < FUZZ_PID_NUM && focus_pids[i]; i++) {
		if (task_pid_nr(current) == focus_pids[i])
			return true;
	}
	return false;
}

void log_cap(void *dst, void *src, unsigned long len, const u8 index) {
	if (is_local_focus_equal()) {
		struct log_cap_info lci;

		lci.stIndex = index;

		if (dst == NULL && src == NULL) {	// alloc
			lci.isAlloc = true;
			printk("[alloc] {len: 0x%lx, st: 0x%x}\n", len, index);
		}
		else {	// copy
			lci.isAlloc = false;
			char dstDesc[256];
			char srcDesc[256];
			bool dstRes = true, srcRes = true;

			snprintf(dstDesc, sizeof(dstDesc), "#%u<dst>", index);
			snprintf(srcDesc, sizeof(srcDesc), "#%u<src>", index);

			dstRes &= find_cache_from_address(dst, dstDesc);
			srcRes &= find_cache_from_address(src, srcDesc);
			printk("[copy] {len: 0x%lx, st: #%u}\n", len, index);
		}
		insert_infos(&lci);
	}
}
...
SYSCALL_DEFINE0(myinit)
{
	...
	*ptr = 'A';
	for (i = 0; i < FUZZ_PID_NUM && focus_pids[i] != 0; i++);
	if (i < FUZZ_PID_NUM) focus_pids[i] = task_pid_nr(current);
	printk("set focus_pid:%d\n", focus_pids[i]);
...
SYSCALL_DEFINE1(get_log_cap, struct log_cap_info __user *, _info)
{
	// clear infos
	if (_info == NULL) {
	...
	// get infos
	if (copy_to_user(_info, infos.buf, 
		infos.size * sizeof(struct log_cap_info))) {
		return -EFAULT;
	}
