...
bool AllocAnalyzerPass::isCall2Copy(std::string calleeName) {
...
bool AllocAnalyzerPass::isMbufData(Value *buf) {
...
bool AllocAnalyzerPass::allocFromHeap(llvm::Instruction* inst) {
    // TODO: analyze a inst with data flow:
    // (1) from some head alloced object;
    // (2) from a CallInst, which call k*alloc().
    return false;
}

void AllocAnalyzerPass::analyzeMptr(llvm::CallInst* callInst, std::string calleeName) {
    ...
    if (calleeName.find("memcpy") != string::npos)
        from = callInst->getArgOperand(1);

    setupPtrInfo(toSet, callInst, from);
}

void AllocAnalyzerPass::analyzeMlen(llvm::CallInst* callInst, std::string calleeName) {
...
bool AllocAnalyzerPass::getAllocSite(std::vector<Value *> &argSet, std::set<CallInst *> &retSet) {
    for (auto *V : argSet) {
...
void AllocAnalyzerPass::analyzeLeak(llvm::CallInst* callInst, std::string calleeName) {
...
void AllocAnalyzerPass::findPtrSources(Value* V, std::vector<llvm::Value *> &srcSet,
...
void AllocAnalyzerPass::findLenSources(Value* V, std::vector<llvm::Value *> &srcSet, 
    std::set<llvm::Value* > &trackedSet) {
...
void AllocAnalyzerPass::addCopyInst(StructInfo *stInfo, llvm::CallInst *callInst, 
...
void AllocAnalyzerPass::setupPtrInfo(std::vector<Value*> &srcSet, CallInst *callInst, Value *from){
    for (std::vector<llvm::Value*>::iterator i = srcSet.begin(), 
        ...
        if(auto *LI = dyn_cast<LoadInst>(V)) {
            ...
            while(auto *GEP = dyn_cast<GetElementPtrInst>(lValue)){
                ...
                // KA_LOGS(0, "[$] check GEP " << *GEP << "\n");
                // if (stInfo->fieldAllocGEP.count(GEP) == 0) break;
...
void AllocAnalyzerPass::setupLenInfo(std::vector<Value*> &lenSet, CallInst *callInst,
...
void AllocAnalyzerPass::setupLeakInfo(std::vector<Value*> &srcSet, CallInst *callInst, Value *from){
...
void AllocAnalyzerPass::setupSiteInfo(std::vector<llvm::Value*> &srcSet, 
...
llvm::StructType* AllocAnalyzerPass::checkSource(std::vector<llvm::Value*>& srcSet, StructTypeSet& stSet , CallInst *callInst, bool isLen) {
...
void AllocAnalyzerPass::dumpSimplifiedLeakers(){
...
void AllocAnalyzerPass::dumpLeakers() {
